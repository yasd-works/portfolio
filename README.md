...
白とブルーを基調に、「その『仕方ない』を、技術で変える。」というメッセージを中心に構成しています。ヘッダーと全セクションに共通の最大幅を使い、冒頭には「航空の現場 × データサイエンス × Web開発」を掲載。実績は1件ずつ、左揃えで成果、業務上の意味、担当範囲、詳細へ読み進められます。航空会社と副業の実績を明記し、詳細は標準HTMLの開閉要素で読めます。

F1スコアと処理時間はBefore / Afterを大きく表示。API連携と情報ポータルには、技術と役割を抜粋した概略図を添えています。ヒューマンファクターズとタイムパフォーマンスは独立した「大切にすること」セクションで、実際の改善事例に結び付けて説明しています。

故障予測のF1スコアは直近の1モデル、PV・MAUは職務経歴書更新時点の直近30日間の概数です。8時間→1時間はデータ抽出・処理の経過時間で、手作業の工数とは区別しています。数値を更新するときは対象と集計時点も合わせて修正してください。

構成図で使う技術の役割は、[Cloudflare D1の公式説明](https://developers.cloudflare.com/d1/)と[Cloudflare R2の公式説明](https://developers.cloudflare.com/r2/)を確認しています。図は主要構成の概略であり、詳細な通信経路や認証方式まで表すものではありません。
