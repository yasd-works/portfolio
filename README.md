...
白とブルーを基調に、「その『仕方ない』を、技術で変える。」というメッセージを中心に構成しています。ヘッダーと全セクションに共通の最大幅を使い、冒頭はキャッチコピー、説明、実績へのリンクを縦に配置。実績は1件ずつ、左揃えで概要、成果、詳細へ読み進められます。長文の行長は外枠の内側で調整し、詳細は標準HTMLの開閉要素で読めます。
